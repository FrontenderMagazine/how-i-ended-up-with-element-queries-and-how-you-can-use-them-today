...
Какое-то время я испытывал трудности с тем, **на что способен CSS**. Те, кто
разрабатывает адаптивные веб-дизайны, охотно подтвердят, что в CSS есть
разочаровывающие недостатки, заставляющие применять препроцессоры, плагины
и прочие утилиты для написания тех стилей, которые при помощи одного только CSS
написать невозможно. Но даже эти утилиты обладают вовсе не безграничными
возможностями.
...
Другая проблема с препроцессорами — это то, что любая ошибка в краткой
...
Может быть, мы используем кучу плагинов для достижения немногих целей,
...
С неё началось обсуждение возможных способов обойти недостатки CSS при помощи
плагинов и полифиллов.

С тех пор, покуда мы ждём улучшения возможностей CSS, появился ряд плагинов,
...
целью стала разработка адаптивного интерфейса, который одинаково подходил бы и
...
все контрольные точки моих медиавыражений стали *бесконтрольными*. Они
...
блоки CSS внутрь конструкций `if` в JavaScript, создающего этакого монстра
...
Повторюсь, у сайта, построенного из HTML, CSS и JavaScript, роль стяжек
...
Уже обладая видением того, какой синтаксис мне требуется, в концу 2014
...
При создании интерфейсных компонентов и виджетов медиавыражения часто
ограничивают разработчиков. Нам часто приходится делать выбор между тем,
чтобы создавать множество различных раскладок, настраиваемых при помощи плагина,
и тем, чтобы сделать интерфейс как можно проще, так что одно решение будет
...
определения. Можно даже не писать `$it`, вот такой код будет делать то же
самое:
...
веб-инспектора, отображающих рассчитанные стили. Сейчас отладка и разработка
...
макету и упрощая тем самым их повторное использование на других страницах и в
...
через подключённый скрипт с типом `text/eqcss`, если в CSS когда-нибудь появится
...
Использование выражений для элементов позволяет работать с обособленными
